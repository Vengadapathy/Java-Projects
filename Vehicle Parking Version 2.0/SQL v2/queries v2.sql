...
select * from vehicleparking	
		where outtime is null and (datediff( (now()),date(intime)) >= 7);

select datediff( (now()),date(intime) ) from vehicleparking ;


select * from parkingslot 
	left join floors on floors.floorid = parkingslot.floorid
    left join vehicletypes on vehicletypes.vehicletypeid = floors.floortypeid
    left join vehicleparking 
    where outtime is not null and (outtime is null and intime is null) ;